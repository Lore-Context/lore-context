...
| npm MCP server package | Published and fresh-install verified | npm org `@lore-context` exists; `@lore-context/server@0.6.0-alpha.0` package page is public; `npm view` returns package metadata; `npm dist-tag ls` returns `alpha` and `latest`; fresh install from a temporary directory succeeded; the installed `lore-context-server` MCP SDK transport returned 11 tools from `tools/list`. |
