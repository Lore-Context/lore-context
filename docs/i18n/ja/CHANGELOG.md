...
- すべての P0 監査結果（ループバックバイパス / ダッシュボード認証 / レート制限 / デモシークレット）がクリアされました。完全な監査証跡については `Lore_Context_项目计划书_2026-04-27.md` と `.omc/plans/lore-prelaunch-fixes-2026-04-28.md` を参照してください。
...
このリリースは、構造化された監査計画に対する並列サブエージェント実行を含む 1 日の本番ハードニングスプリントの結果です。計画と監査アーティファクトは `.omc/plans/` に保存されています。
