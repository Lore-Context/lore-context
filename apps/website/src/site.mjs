...
function publishedReferenceChart() {
  return `<div class="reference-chart" aria-label="External published LoCoMo reference chart">
    <div class="reference-head"><strong>External published references</strong><span>LoCoMo answer accuracy · LLM-as-Judge or vendor reports · not directly comparable</span></div>
    <div class="reference-grid">${publishedMemoryReferences.map(referenceRow).join("")}</div>
    <p class="chart-note">These rows show market context from public reports. They were not rerun in Lore's retrieval-only harness, so they should not be read as a direct Lore ranking.</p>
  </div>`;
}

function referenceRow(item) {
  const width = Math.max(0, Math.min(100, item.width));
  return `<div class="reference-row"><span>${item.label}</span><b><i style="width:${width}%"></i></b><em>${item.value}</em><small>${item.source}</small></div>`;
}
